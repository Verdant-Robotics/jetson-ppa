...
if [ "${PLATFORM}" == "jetson-r32" ]; then
  DEB_FILENAME="aws-sdk-cpp_${VERSION}_arm64.deb"
elif [ "${PLATFORM}" == "x64" ]; then
  DEB_FILENAME="aws-sdk-cpp_${VERSION}_amd64.deb"
fi
BUILD_DIR="${SCRIPT_DIR}/../build/awssdk-${PLATFORM}"
if [ ! -f ${BUILD_DIR}/${DEB_FILENAME} ]; then
  echo "${BUILD_DIR}/${DEB_FILENAME} not found. Run ./scripts/build.sh ${PLATFORM} awssdk first."
  exit 1
fi
