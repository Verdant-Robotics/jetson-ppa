...
S3_BUCKET=repo.download.martian.ag
VERSION=4.4.2
...
if [ "${PLATFORM}" == "x64" ]; then
  DEB_FILENAME="ffmpeg_${VERSION}_amd64.deb"
else
  DEB_FILENAME="ffmpeg_${VERSION}_arm64.deb"
fi
BUILD_DIR="${SCRIPT_DIR}/../build/ffmpeg-${PLATFORM}"
if [ ! -f ${BUILD_DIR}/${DEB_FILENAME} ]; then
  echo "${BUILD_DIR}/${DEB_FILENAME} not found. Run ./scripts/build.sh ${PLATFORM} ffmpeg first."
  exit 1
fi
...
# Upload the ffmpeg deb package to S3
docker run --rm \
  -v ~/.aws:/root/.aws \
  -v ${BUILD_DIR}:/build \
